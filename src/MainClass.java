 ...
 *  		불특정 다수의 개체를 저장하기에는 문제가됨
 *  		2) 객체 삭제했을 때 해당 인덱스가 비워지게 됨.
 ...
 *  	-제네릭 : 특정 타입만 다루지 않고, 여러 종류의 타입으로 변신할 수있도록
 *  				클래스나 메소드를 일반화시키는 기법
 ...
 *  List 특징 = 순서를 유지하고 있다.(객체마다 index를 가지고 있음)
 *  			= 중복을 허용한다.
 *  			= 데이터베이스 프로그램
 *  
 *  	set => TreeSet, HashSet
 ...
 *  		insert ()
 *  	=remove(index) =>데이터 삭제
 *  	Delete ~ SET  //특정조건을~ 삭제한다
 *  	delete()
 *  	
 *  =set(index,객체명) => 데이터 수정
 *  	UPDATE ~ SET  //특정조건에 해당하는걸 수정하겟다~
 *  	update()
 *  
 *  =get(index) = 데이터 검색
 *  	SELECT ~ FROM
